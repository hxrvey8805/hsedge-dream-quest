...
export const EquityCurve = ({ refreshTrigger, viewMode = 'profit', monthSwitchEnabled = false, currentMonth = new Date(), selectedAccountId = null }: EquityCurveProps) => {
  ...
  useEffect(() => {
    fetchEquityData();
  }, [refreshTrigger, viewMode, monthSwitchEnabled, currentMonth, selectedAccountId, settings.defaultRiskAmount, settings.monthlyRiskOverrides]);
    ...
    if (trades && trades.length > 0) {
      ...
      const equityPoints: EquityPoint[] = trades.map((trade: Trade, index: number) => {
        const value = viewMode === 'rMultiple' 
          ? calculateRMultiple(trade.profit, trade.risk_to_pay, settings.defaultRiskAmount, trade.trade_date, settings.monthlyRiskOverrides)
          : (trade.profit || 0);
        cumulative += value;
